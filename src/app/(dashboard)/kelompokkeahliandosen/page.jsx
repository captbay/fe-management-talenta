...
import { getKelompokKeahlian } from "@/api/api";
...
const Page = () => {
  ...
  const [labelKelompokKeahlianApi, setLabelKelompokKeahlianApi] = useState([]);
  const [dataKelompokKeahlianApi, setDataKelompokKeahlianApi] = useState([]);
  const [Dosen, setDosen] = useState([]);
  const [filteredDosen, setFilteredDosen] = useState([]);

  useEffect(() => {
    getKelompokKeahlian(token)
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          const labels = [];
          const data = [];
          res.data.data.forEach((item) => {
            labels.push(item.kelompok_keahlian_name);
            data.push(item.jumlah);
          });

          setLabelKelompokKeahlianApi(labels);
          setDataKelompokKeahlianApi(data);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const dataKelompokKeahlian = {
    labels: labelKelompokKeahlianApi,
    datasets: [
      {
        label: "Jumlah Dosen",
  ...
  const onClick = (event) => {
    ...
    if (data.length > 0) {
      ...
      const selectedKelompokKeahlian = labelKelompokKeahlianApi[index];

      // Filter Dosen based on the selected program with exact case match
      const filtered = Dosen.filter(
        (dosen) =>
          dosen.kelompok_keahlian?.toLowerCase() ===
  ...
  return (
    <section className="flex flex-col">
      <div className="h-[60vh] w-[60vw] relative md:w-full md:min-h-[70vh]">
        {labelKelompokKeahlianApi.length > 0 &&
          dataKelompokKeahlianApi.length > 0 && (
            <Bar
              options={options}
              data={dataKelompokKeahlian}
              onClick={onClick}
              ref={chartRef}
            />
          )}
            ...
            {filteredDosen.length === 0 ? (
              <tr>
                <td colSpan="7" className="text-center px-6 py-4">
                  Silahkan klik bar untuk melihat data dosen berdasarkan
                  kelompok keahlian
                </td>
              </tr>
            ) : (
              filteredDosen.map((k, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{k.kelompok_keahlian}</td>
