...
import { getJabatan } from "@/api/api";
...
const Page = () => {
  ...
  const [labelJabatanFungsionalApi, setLabelJabatanFungsionalApi] = useState(
    []
  );
  const [dataJabatanFungsionalApi, setDataJabatanFungsionalApi] = useState([]);
      ...
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          ...
          res.data.data.forEach((item) => {
            labels.push(item.jabatan_name);
            data.push(item.jumlah);
          });

          setLabelJabatanFungsionalApi(labels);
          setDataJabatanFungsionalApi(data);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const dataJabatanFungsional = {
    labels: labelJabatanFungsionalApi,
    datasets: [
      {
  ...
  const onClick = (event) => {
    ...
    if (data.length > 0) {
      ...
      const selectedJabatanFungsional = labelJabatanFungsionalApi[index];

      // Filter Pegawai based on the selected program with exact case match
      const filtered = Pegawai.filter(
        (pegawai) =>
          pegawai.jabatan_fungsional?.toLowerCase() ===
